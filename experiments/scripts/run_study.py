...
For each hypothesis × condition × scenario combination not yet registered in
...
from study_io import load_study_definition  # noqa: E402
    ...
    for hyp_id, hyp in data["hypotheses"].items():
        ...
        for cond_name, cond in hyp["conditions"].items():
            ...
            for scenario in scenarios:
                ...
                if dry_run:
                    iv = hyp["independent_variable"]
                    eval_dir = (
                        PROJECT_ROOT / "outputs" / f"eval_{study_name}"
                        / hyp_id / f"{iv}={cond_name}" / scenario
                    )
                    run_cmd(
                        ["uv", "run", "python", str(eval_script.relative_to(PROJECT_ROOT)),
                         "<checkpoint>", "-o", str(eval_dir.relative_to(PROJECT_ROOT))],
                        dry_run=True,
                    )
                ...
                eval_dir = (
                    PROJECT_ROOT / "outputs" / f"eval_{study_name}"
                    / hyp_id / f"{hyp['independent_variable']}={cond_name}" / scenario
                )
                ...
                # eval.py writes eval_step_{N}_checkpoint.json into the output dir
                eval_out = eval_dir / f"eval_{checkpoint.name}"

                # Stage 3: Register
                register_run(study_path, hyp_id, cond_name, scenario, source_dir, eval_out)
                print(f"    registered in study.yaml")

    # Stage 4: Organize
    organizer = PROJECT_ROOT / "experiments" / "scripts" / "organize_experiments.py"
    print(f"\nStage 4: organize")
    run_cmd(
        ["uv", "run", "python", str(organizer.relative_to(PROJECT_ROOT)),
         str(study_path.relative_to(PROJECT_ROOT))],
        dry_run=dry_run,
    )
